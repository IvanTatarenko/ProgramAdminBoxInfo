...
namespace ProgramAdminBoxInfo
{
    ...
    {
        public void Insert_query(string name_usa, string name_ua = "", string height = "", string reach = "", string stance = "", string wiki_url_en = "", string wiki_url_ua = "", string boxreg_url = "", string nationality = "", string residence = "", string birth_place = "")
        {
            //connection to the database
            DB db = new DB();
            MySqlCommand command = new MySqlCommand("INSERT INTO `boxers` (`name_usa`, `name_ua`, `height`, `reach`, `stance`, `wiki_url_en`, `wiki_url_ua`, `boxreg_url`, `nationality`, `residence`, `birth_place`) VALUES (@name_usa, @name_ua, @height, @reach, @stance, @wiki_url_en, @wiki_url_ua, @boxreg_url, @nationality, @residence, @birth_place)", db.getConnection());
            //mask requests
